...
use crate::domain::{
    ...
    proof::{normal::MerkleProofNormal, MerkleProofTrait},
};
...
pub struct MerkleTreeNormal<Method: HashMethod> {
    pub tree: Vec<Vec<u8>>, // array-based complete binary tree
    pub root: Vec<u8>,      // cached root
    method: PhantomData<Method>,
}

impl<Method: HashMethod> MerkleTreeTrait<Method> for MerkleTreeNormal<Method> {
    ...
    fn from_leaves_hashes(leaves: Vec<Vec<u8>>) -> Result<Self> {
        Self::from_leaves_hashes_base(leaves)
    }
...
impl<Method: HashMethod> MerkleTreeNormal<Method> {
    pub fn get_proof(&self, index: usize) -> Result<MerkleProofNormal<Method>> {
        ...
        let proof = MerkleProofNormal::new(path);
...
pub type MerkleTreeNormalSha256 = MerkleTreeNormal<Sha256Normal>;
...
mod tests {
    use crate::domain::{
        ...
        tree::{normal::MerkleTreeNormalSha256, MerkleTreeTrait},
    };
    use rand::Rng;
    use std::sync::Arc;
    use std::thread;
    ...
    fn test_empty_leaves_error() {
        ...
        let result = MerkleTreeNormalSha256::from_leaves_data(leaves);
        assert!(
            result.is_err(),
    ...
    fn test_single_leaf_tree() {
        ...
        let tree = MerkleTreeNormalSha256::from_leaves_data(leaves.clone())
            .expect("Should build a single-leaf tree successfully");
        let root = tree.get_root();

        let expected = Sha256Normal::hash_leaf(b"only_leaf");
        assert_eq!(
            root, expected,
    ...
    fn test_random_leaves() {
        ...
        for &size in &sizes {
            ...
            let tree = MerkleTreeNormalSha256::from_leaves_data(random_leaves.clone())
            ...
            for _ in 0..3 {
                ...
                let leaf_hash = Sha256Normal::hash_leaf(&random_leaves[i]);
                let index = tree
    ...
    fn test_proof_out_of_range_index() {
        ...
        let tree = MerkleTreeNormalSha256::from_leaves_data(leaves).unwrap();
    ...
    fn test_proof_missing_data() {
        ...
        let missing_hash = Sha256Normal::hash_leaf(b"not in the tree!");
    ...
    fn test_duplicate_leaves() {
        ...
        let tree = MerkleTreeNormalSha256::from_leaves_data(leaves.clone()).unwrap();
        assert_eq!(
        ...
        let dup_hash = Sha256Normal::hash_leaf(b"dup");
    ...
    fn test_very_large_leaf_data() {
        ...
        let hashed_leaf = Sha256Normal::hash_leaf(&leaves[0]);
        assert!(
    ...
    fn test_from_leaves_hashes() {
        ...
        let hashed_leaves: Vec<Vec<u8>> = raw_leaves
            .iter()
            .map(|l| Sha256Normal::hash_leaf(l))
            .collect();

        // Build from prehashed
        let tree = MerkleTreeNormalSha256::from_leaves_hashes(hashed_leaves.clone())
    ...
    fn test_wrong_root_fails() {
        ...
        let tree =
            MerkleTreeNormalSha256::from_leaves_data(leaves.clone()).expect("Must build tree");

        let correct_root = tree.get_root();
        let wrong_root = Sha256Normal::hash_leaf(b"totally_wrong_root");

        // Generate a proof for "another_leaf"
        let idx = tree.get_index_by_data(b"another_leaf").unwrap();
        let proof = tree.get_proof(idx).unwrap();
        let leaf_hash = Sha256Normal::hash_leaf(b"another_leaf");
    ...
    fn test_malicious_sibling_replacement() {
        ...
        let tree =
            MerkleTreeNormalSha256::from_leaves_data(leaves.clone()).expect("Tree must build fine");
        let root = tree.get_root();
        ...
        if !proof.steps.is_empty() {
            let fake_sibling = Sha256Normal::hash_leaf(b"FAKE_SIBLING");
            proof.steps[0].1 = fake_sibling;
        }

        // Now the proof must fail
        let leaf_hash_b = Sha256Normal::hash_leaf(b"leafB");
        assert!(
    ...
    fn test_malicious_direction_flip() {
        ...
        let correct_leaf = Sha256Normal::hash_leaf(b"left");
    ...
    fn test_incomplete_proof_fails() {
        ...
        let leaf_hash_two = Sha256Normal::hash_leaf(b"two");
        assert!(
    ...
    fn test_large_number_of_leaves_stress() {
        ...
        for _ in 0..5 {
            ...
            let leaf_hash = Sha256Normal::hash_leaf(&leaves[idx]);
            let proof = tree
    ...
    fn test_cannot_insert_extra_leaves_after_construction() {
        ...
        let tree = MerkleTreeNormalSha256::from_leaves_data(leaves).expect("Must build tree");
        ...
        let new_tree =
            MerkleTreeNormalSha256::from_leaves_data(new_leaves).expect("New tree built");
        ...
        for _ in 0..10 {
            ...
            let handle = thread::spawn(move || {
                ...
                for _ in 0..20 {
                    ...
                    let leaf_hash = Sha256Normal::hash_leaf(&leaves_copy[idx]);
                    let proof = tree_ref
    ...
    fn test_duplicate_leaves_blockchain_scenario() {
        ...
        for _i in 0..4 {
            let proof = tree
                ...
                .unwrap();
            let leaf_hash = Sha256Normal::hash_leaf(b"tx");
            assert!(
                proof.verify(&root, &leaf_hash),
    ...
    fn test_cannot_tamper_root() {
        ...
        let tree = MerkleTreeNormalSha256::from_leaves_data(leaves.clone()).expect("Tree builds");
        let correct_root = tree.get_root();
        ...
        let proof = tree.get_proof(idx).unwrap();
        let leaf_hash = Sha256Normal::hash_leaf(b"block_tx_1");
    ...
    fn test_large_data_and_many_leaves_efficiency() {
        ...
        for i in [
            ...
            let leaf_hash = Sha256Normal::hash_leaf(&leaves[i]);
    ...
    fn test_invalid_leaf_in_blockchain() {
        ...
        let empty_hash = Sha256Normal::hash_leaf(b"");
    ...
    fn test_no_sideload_in_proof() {
        ...
        let random_leaf = Sha256Normal::hash_leaf(b"some_injected_sibling");
        ...
        let leaf_hash_bar = Sha256Normal::hash_leaf(b"bar");
        assert!(
    ...
    fn test_forced_mismatch_proof_fails() {
        ...
        let mut tree = MerkleTreeNormalSha256::from_leaves_data(leaves).unwrap();
    ...
    fn test_parent_index_of_root_fails() {
        let res = MerkleTreeNormalSha256::parent_index(0);
        assert!(
            res.is_err(),
    ...
    fn test_sibling_index_of_root_fails() {
        let res = MerkleTreeNormalSha256::sibling_index(0);
        assert!(
            res.is_err(),
